# Task = create hex grids for camera trap design
# Adapted from AJB 01/11/2019
library(sp); library(lattice); library(rgdal); library(rgeos); library(raster)

write_shapefiles <- F

# Define coordinate reference system: example as GDA94
...
# Load region boundary as a shapefile: example with boundary of SilverPlain in Tasmania
dsn <- getwd()
